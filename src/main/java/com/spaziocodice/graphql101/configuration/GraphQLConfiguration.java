    ...
    public ExecutionInputCustomizer executionInputCustomizer() {
        return (executionInput, webRequest) -> {
            executionInput.getGraphQLContext()
                    // Here, the Authorization object that is put into the GraphQLContext should come from a WebRequest's attribute:
                    // <code>.put("auth", webRequest.getAttribute("attribute name", RequestAttributes.SCOPE_REQUEST))</code>
                    // However, for the sake of simplicity within our example, we are using the AuthHelper class to
                    // provide that object.
